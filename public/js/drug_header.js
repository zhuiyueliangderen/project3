...
search.onclick=function(){
	...
	xhr.onreadystatechange=function(){
		if(xhr.readyState==4&&xhr.status==200){
			...
			//在父页面打开新页面
			//绝对路径实现跨域
			parent.location.href="http://127.0.0.1:5500/public/drug_details.html?did="+did;
		}
	}
	//3.打开连接创建请求
...
if(lastPage=="user_login"){
	...
	if(uname){//如果uname存在
		...
		alert("请先登录");
		parent.location.href="http://127.0.0.1:5500/public/user_login.html";
	}
	//查找已经添加的a元素  退出 
	...
	if($uname){//如果$uname存在
		...
		alert("请先注册");
		parent.location.href="http://127.0.0.1:5500/public/user_register.html";
	}
	//查找已经添加的a元素  退出 
	var a=document.querySelector("div.right_nav>span:first-child>a");
	a.onclick=function(){
